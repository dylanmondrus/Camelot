...
public class MyEdgeBuilder extends NodeBuilder {
	...
	public void GoToFountainEdges() {
		var node = get(MyNodeLabels.Barrell.toString());
		var nextNode = get(MyNodeLabels.Fountain.toString());
		var choice = PlayerInteraction(MyStoryEntities.doug, MyChoiceLabels.WalkToFountain.toString(),MyStoryEntities.fountain);
		node.add(new Edge(choice, nextNode));
	}
	@BuilderMethod
	public void InteractBeggar() {
		...
		var nextNode = get(MyNodeLabels.Fountain.toString());
		var choice = PlayerInteraction(MyChoiceLabels.TalkToBeggar.toString(), MyStoryEntities.beggar, Icons.talk, "Talk to Beggar");
		node.add(new Edge(choice, nextNode));
	}
	@BuilderMethod
	...
	public void WalkToMerchantBillEdges() {
		...
		var choice = PlayerInteraction(MyChoiceLabels.WalkToMerchantBill.toString(),MyStoryEntities.doug, MyStoryEntities.merchantbill);
		node.add(new Edge(choice, nextNode));
	}
	...
	public void AcceptQuestActions() {
		...
		var choice = new PlayerInteraction(MyChoiceLabels.gotoForestPath.toString(),MyStoryEntities.doug,MyStoryEntities.forestpath);
		node.add(new Edge(choice, nextNode));
	...
	public void ContinueDrinkingEdges() {
		...
		new PlayerInteraction(MyChoiceLabels.GoToFireplaceChoice.toString(), MyStoryEntities.doug, Icons.fireplace, "Look at Fireplace");
		node.add(new Edge(choice1, nextNode1));
		var nextNode2 = get (MyNodeLabels.VisitTavernTable);
		var choice2 = new PlayerInteraction(MyChoiceLabels.VisitTavernTableChoice.toString(), MyStoryEntities.doug, Icons.taverntable, "Investigate Tavern Table");
		node.add(new Edge(choice2, nextNode2));
	...
	public void VisitTavernTableEdges() {
		...
		var choice1 = new PlayerInteraction(MyChoiceLabels.PickupTorchChoice.toString(), Icons.taverntable, "Pickup Torch");
		node.add(new Edge(choice1, nextNode1));
		var nextNode2 = get(MyNodeLabels.DontPickupTorch.toString());
		var choice2 = new PlayerInteraction(MyChoiceLabels.PickupTorchChoice.toString(), Icons.taverntable, "Dont Pickup Torch");
		node.add(new Edge(choice2, nextNode2));
	}
	@BuilderMethod
